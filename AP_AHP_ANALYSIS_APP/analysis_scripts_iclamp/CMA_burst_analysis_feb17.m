...
combinedsweep = combine_sweeps(dataallsweeps); %combine the data for the whole trace in 1 single sweep (array)

%display(combinedsweep);
plot(combinedsweep);
b = 1;
...
numspikes = 0;
%dV_thresh = 100 * 8e-6 / si_actual; %THRESHOLD SLOPE  ( 0.8 mV / ms) (update: )
%dV_thresh = 15/ms_to_sampleunits(si, 1); %10mV/ms.  marian: 4
